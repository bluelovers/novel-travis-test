...
async function cacheDiffNovelList(data) {
    ...
    Object.keys(data.list)
        ...
        if (pathMain.match(/^\./) || ['docs'].includes(pathMain)) {
            log_1.default.log('[SKIP (cacheDiffNovelList)]', pathMain, Object.keys(data.list[pathMain]));
            return;
        }
        Object.keys(data.list[pathMain]).forEach(function (novelID) {
            ...
            if (arr.length) {
                ...
                log_1.default.log('[SKIP (cacheDiffNovelList)]', pathMain, novelID, data.list[pathMain][novelID].length);
            }
...
async function cacheFileList(data) {
    if (data.pathMain.match(/_out$|^\./) || ['docs'].includes(data.pathMain)) {
        log_1.default.log('[CACHE (cacheFileList)]', 'SKIP: ', data.pathMain, data.novelID);
        return;
    }
